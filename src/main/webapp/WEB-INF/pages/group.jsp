...
<html>
    ...
    <script src="<%=path%>/res/bootstrap-datetimepicker/js/moment-with-locales.min.js"></script>  
	...
	<body class="hold-transition skin-blue sidebar-mini">
		<div class="wrapper">
			...
			<div class="content-wrapper">
				...
				<section class="content-header">
					...
					<a href="<%=path%>/group/list"><sp:message code="menu.group"/></a>&nbsp;&nbsp;<small><sp:message code="group.list"/></small>
					<!-- 
					<ol class="breadcrumb">
				...
				<section class="content">
					...
					<div class="row-fluid">
						...
						<div class="row">
							<form id="queryForm" action="<%=path%>/group/list" method="post">
								<div class="col-xs-2">
									...
									<table id="dataTable" class="table table-striped table-bordered table-hover table-condensed" align="center">
										<thead>
											<tr class="info">
												...
												<th><sp:message code="group.name"/></th>
											  <th><sp:message code="sys.create.time"/></th>
											  <th><sp:message code="group.rake.back"/></th>
											  <th><sp:message code="group.total"/></th>
											  <th><sp:message code="group.back"/></th>
											  <th><sp:message code="group.pay"/></th>
											  <th><sp:message code="group.netvalue"/></th>
			...
			<div class="modal-dialog" >
				<div class="modal-content">
					...
					<div class="modal-body" >
						<form class="form-horizontal"  id="editForm" action="<%=path%>/group/update" method="post">
							<input type="hidden" class="form-control" name="groupId">
							<div class="form-group">
								<label for="inputName" class="col-sm-3 control-label"><sp:message code="group.name"/></label>
								<div class="col-sm-9">
									<input type="text" class="form-control" name="group_name">
								</div>
							</div>
							<div class="form-group">
								<label for="inputEmail" class="col-sm-3 control-label"><sp:message code="group.rake.back"/></label>
								<div class="col-sm-9">
									<input type="text" class="form-control" name="rake_back">
								</div>
							</div>
						</form>
					</div>
			...
			$(function () {
				...
				var tables = $("#dataTable").dataTable({
	                ...
	                columns: [//对应上面thead里面的序列
	                    ...
	                    {"data": null},
	               		{"data": 'group_name'}, //mData 表示发请求时候本列的列明，返回的数据中相同下标名字的数据会填充到这一列
	               		{"data": 'create_time', 
	                ...
	                columnDefs: [
		                ...
		                },*/
		                {
		                    ...
		                    defaultContent: " <div class='btn-group'>"+
		                    				...
		                    				"<button id='configRow' class='btn btn-primary btn-sm' type='button'><i class='fa fa-cog'></i></button></div>"
		                }
	                ],
	              	//每加载完一行的回调函数
	    ...
	    　　　　　　　　	//$('td', row).eq(5).css('font-weight',"bold").css("color","red");//获取到具体行具体格的元素
				...
				$('#dataTable tbody').on( 'click', '#editRow', function () {
					...
					$("input[name=groupId]").val(data.groupIdStr);
					$("input[name=group_name]").val(data.group_name);
					$("input[name=rake_back]").val(data.rake_back);
				...
				$('#dataTable tbody').on( 'click', '#delRow', function () {
		            ...
		            if(confirm("是否确认删除这条数据?")){
		                $.ajax({
		                    url:'<%=path%>/group/del/'+data.groupIdStr,
		                    type:'delete',
		                    dataType: "json",
		                    //timeout:"3000",
		                    cache:"false",
				...
				$('#dataTable tbody').on( 'click', '#configRow', function () {
					var data = tables.api().row($(this).parents('tr')).data();
					alert(data.groupIdStr);
					location.href='<%=path%>/device/list/'+data.groupIdStr;
		        });
